...
| X-Download-Token | string | *(opaque)* | [Token](tokens.md) required to download original files; reissued on every request, so read it per response rather than storing it |
| X-Preview-Token  | string | 174utza5   | [Token](tokens.md) required for the [Thumbnail Image API](thumbnails.md#image-endpoint-uri)                                       |
