...
Generated thumbnail files are stored in the `storage/cache/thumbnails` folder, where the path and file name depend on the [thumbnail type](#thumbnail-sizes) and original file hash, e.g.:
