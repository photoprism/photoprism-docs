...
| `WORKING_DIR`          | `/go/src/github.com/photoprism/photoprism` | In-container working directory and bind mount target for the checkout. Reused for the Dockerfile `WORKDIR` build arg, `working_dir`, the source bind mount, and every PhotoPrism path variable so they stay in sync. |
...
[^1]: Instead of using Docker, you can also set up your own build environment based on the steps documented in the [Dockerfiles](https://github.com/photoprism/photoprism/tree/develop/docker/develop) we provide. For this, you should have at least Go 1.26 (the minimum recorded in [`go.mod`](https://github.com/photoprism/photoprism/blob/develop/go.mod)), TensorFlow for C, the ONNX Runtime, Make, Node.js 22.15 (the minimum required by `frontend/scripts/precompress.js`, which uses the built-in `node:zlib` zstd API to emit `.gz`/`.zst` siblings during `make build-js`), NPM 11, and MariaDB 12 installed. The ONNX Runtime is loaded as a shared library at startup rather than linked into the binary, so install it with [`scripts/dist/install-onnx.sh`](https://github.com/photoprism/photoprism/blob/develop/scripts/dist/install-onnx.sh), which pins the version the bindings expect; see [Running Unit and Acceptance Tests](tests.md#onnx-runtime) for what an outdated one looks like. Note that the test results will be unreliable without Docker. This method is therefore not well suited for contributors and we cannot provide support if something does not work as expected.
