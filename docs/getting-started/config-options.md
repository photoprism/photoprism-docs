...
!!! example ""
    Some options, such as those for [hardware transcoding](advanced/transcoding.md), are [only available to members](https://www.photoprism.app/membership) due to the time required for testing, maintenance, and providing support. They are tagged in their description as either an *essentials* or *plus* feature.
