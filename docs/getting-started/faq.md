...
To free up as much space as possible, the most effective way is to delete all files in the `/cache/thumbnails` *storage* folder. It is located outside the *originals* folder by default, depending on [your configuration](config-options.md). Then perform a full rescan of your library or run the command `photoprism thumbs -f` in a terminal if you have direct server access. This command can also be used to replace existing thumbnails, for example after changing the quality settings. Unneeded or higher resolution thumbnails won't be automatically removed.
