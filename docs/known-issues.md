...
*Libraries created before the new model became available keep the previous one until they are migrated, so they are still affected by both limitations — see [Model Upgrade](#model-upgrade) below.*
...
Migrating an existing library to the current model is a deliberate step: it re-embeds every face, keeps the people you have already identified, and is followed by a short sequence of commands that detects the additional faces the new model can find and settles the clusters. It takes a while on a large library, and the instance must be restarted afterwards. Please read the full procedure before you start, and [create a backup](user-guide/ai/face-recognition.md#creating-a-backup) first, since a migration can only be undone by restoring one.

[Learn more ›](user-guide/ai/face-recognition.md#upgrading-an-existing-library)
