...
!!! note ""
    See [Roles and Permissions](roles.md) for the account roles you can set with `--role`, and which of them your edition provides.  Use `photoprism users add --help` and `photoprism users mod --help` on your instance to see the exact options supported by that build.
